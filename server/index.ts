...
export const CARDANO_NETWORK = process.env.CARDANO_NETWORK || CardanoNetwork.preview;
export const CLAIM_ADMIN_KEY = process.env.CLAIM_ADMIN_KEY || crypto.randomBytes(20).toString('hex');
...
export const VM_KOIOS_URL = process.env.KOIOS_URL_TESTNET || process.env.KOIOS_URL;
...
app.get(
  ...
  errorHandlerWrapper(async (_req: Request, res: Response<GetPoolsDto>) => {
    ...
    const whitelist = CLAIM_FEE_WHITELIST ? CLAIM_FEE_WHITELIST.split(",") : [];
    ...
    if (unlock === "true") {
      if (CLAIM_FEE_WHITELIST) {
        ...
        if (whitelist.includes(accountInfo.delegated_pool)) {
          ...
          vmArgs += `&overhead_fee=${CLAIM_FEE}&unlocks_special=true`;
        }
      } else {
        vmArgs += `&overhead_fee=${CLAIM_FEE}&unlocks_special=true`;
      }
